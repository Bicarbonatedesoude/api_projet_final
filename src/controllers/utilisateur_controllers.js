...
exports.ajouter_Util = (req, res) => {
    const { nom, cle_api, courriel, password } = req.body;

    // Vérification des champs requis
    const messages = [];
    if (!nom) {
        // Si le champ nom est manquant, on ajoute un message
        messages.push("nom");
    }
    if (!courriel) {
...
exports.regenerer_Cle = async(req, res) => {
    const { courriel, password } = req.body;
    ...
    utilisateur.verifCombinaison(req.body)
        .then((user) => {
            ...
            .catch((err) => {
                res.status(500).send({
                    message: err.message || 'Erreur lors de la régénération de la clé API'
                });
            });
        })
        .catch((error) => {
